...
class GradientDescentAlg private[spark](
                                         ...
                                         private var testData: RDD[(Double, Vector)])
  ...
  def runMiniBatch(
    ...
    while (!converged && i <= numIterations) {
      ...
      if (miniBatchSize > 0) {
        ...
        updater match {
          case _: SimpleUpdater =>
            val update = updater.asInstanceOf[SimpleUpdater].compute(
              weights, Vectors.fromBreeze(gradientSum / miniBatchSize.toDouble), learningRate/sqrt(i), i, regParam)
            weights = update._1
            regVal = update._2
          case _: AdamUpdater =>
            val update = updater.asInstanceOf[AdamUpdater].compute(
              weights, Vectors.fromBreeze(gradientSum / miniBatchSize.toDouble), learningRate/sqrt(i), smoothingTerm, beta, betaS,
              i, regParam)
            weights = update._1
            regVal = update._2
          case _: AdagradUpdater =>
            val update = updater.asInstanceOf[AdagradUpdater].compute(
              weights, Vectors.fromBreeze(gradientSum / miniBatchSize.toDouble), learningRate/sqrt(i), smoothingTerm,
              i, regParam)
            weights = update._1
            regVal = update._2
          case _: MomentumUpdater =>
            val update = updater.asInstanceOf[MomentumUpdater].compute(
              ...
              learningRate/sqrt(i), i, regParam)
            weights = update._1
            regVal = update._2
          case _: NesterovUpdater =>
        ...
        accs=testAccuracy(weights, testData);
